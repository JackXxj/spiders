...
def pubg_friends(lock, r, first_hour, fileout):
    while True:
        try:
            current_hour = time.strftime('%H')
            if current_hour != first_hour:  # 退出进程
                print time.strftime('[%Y-%m-%d %H:%M:%S]：pubg_friends()退出')
                return False
            if time.strftime('%M%S') == '0000':
                print time.strftime('当前时间：[%Y-%m-%d %H:%M:%S]')
            data = r.lpop('spider:python:pubg_friends:keyword:dest')
            if data:
                with lock:
                    fileout.write(data)
                    fileout.write('\n')
...
def main():
    ...
    pubg_friends_length = r.llen('spider:python:pubg_friends:keyword:dest')  # redis的数据量
    print time.strftime('[%Y-%m-%d %H:%M:%S]'), 'redis中pubg_friends的数据量：', pubg_friends_length
    lock = threading.Lock()
    ...
    threads = []
    for i in xrange(1):
        t = threading.Thread(target=pubg_friends, args=(lock, r, first_hour, fileout))
        t.start()
        threads.append(t)
