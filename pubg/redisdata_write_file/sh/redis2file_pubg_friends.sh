...
time=`date +"%Y%m%d"`
redis_key='spider:python:pubg_friends:keyword:dest'
file_name=$remote_path'/pubg_friends_'$time

size=$(/usr/local/bin/redis-cli -c -h 172.31.10.132 -p 6379 llen spider:python:pubg_friends:keyword:dest)
