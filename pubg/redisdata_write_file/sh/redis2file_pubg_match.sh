...
time=`date +"%Y%m%d"`
redis_key='spider:python:pubg_match:keyword:dest'
file_name=$remote_path'/pubg_match_'$time

size=$(/usr/local/bin/redis-cli -c -h 172.31.10.132 -p 6379 llen spider:python:pubg_match:keyword:dest)
