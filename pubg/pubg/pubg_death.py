...
MATCH_DATA_QUEUE = Queue.Queue()    # 数据队列(存储一场游戏的赛事)
GAME_DATA_QUEUE = Queue.Queue()    # 数据队列（存储一场游戏中所有团队的信息）
DEATH_DATA_QUEUE = Queue.Queue()    # 数据队列（存储一场游戏死亡信息）
...
def get_redis_proxy():
    ...
    pubg_death_proxy_length = r.llen('spider:pubg_death:proxy:kuai')  # pubg_death
    print time.strftime('[%Y-%m-%d %H:%M:%S]'), 'redis中pubg_death的代理ip长度：', pubg_death_proxy_length
...
def pubg_userid(lock, r):
    ...
    while True:
        ...
        if 001000 >= current_time >= 000000:  # 退出进程
            ...
            return False
        try:
            nick_name = r.lpop('spider:python:pubg_death:keyword')    # 用户昵称
            if nick_name:
            ...
            with lock:
                ...
                r.rpush('spider:python:pubg_death:keyword', nick_name)
    ...
    try:
        ...
        if matches:
		    ...
		    # redis内存不足
                    ...
                    if not r.sismember('spider:python:pubg_death_match_id:keyword:{mid}'.format(mid=match_id_md5[:2]), match_id_md5):
                        r.sadd('spider:python:pubg_death_match_id:keyword:{mid}'.format(mid=match_id_md5[:2]), match_id_md5)
                        user_deaths_url = 'https://pubg.op.gg/api/matches/{match_id}/deaths'.format(match_id=match_id.replace('=', '%3D'))  # 一场游戏中所有团队死亡信息
                        user_death(user_deaths_url, match_id, proxies, r)  # 一场游戏中所有团队死亡信息
		    #user_deaths_url = 'https://pubg.op.gg/api/matches/{match_id}/deaths'.format(match_id=match_id.replace('=', '%3D'))  # 一场游戏中所有团队死亡信息
                    #user_death(user_deaths_url, match_id, proxies, r)  # 一场游戏中所有团队死亡信息
...
def user_death(user_deaths_url, match_id, proxies, r):
    '''
    一场游戏中所有团队死亡信息
    :param user_deaths_url:
    :return:
    '''
    try:
        current_time = int(time.strftime('%H%M%S'))
        if 001000 >= current_time >= 000000:  # 退出进程
            print time.strftime('[%Y-%m-%d %H:%M:%S]：'), 'user_death()退出'
            return False
        print time.strftime('[%Y-%m-%d %H:%M:%S]：'), '一场比赛死亡信息 ',  user_deaths_url, proxies
        response = requests.get(url=user_deaths_url, headers=headers, proxies=proxies, timeout=10)
        # print '请求状态码：', response.status_code
        # print response.text
        response_json = response.json()
        user_deaths = []
        deaths = response_json.get('deaths')  # 赛事游戏死亡数据列表
        if deaths:
            for death in deaths:
                description = death.get('description')  # 死法
                killer = death.get('killer')  # 杀手信息
                if killer:
                    killer_rank = killer.get('rank')  # 杀手排名
                    killer_nickname = killer.get('user').get('nickname')  # 杀手昵称
                else:
                    killer_rank = ''
                    killer_nickname = ''
                victim = death.get('victim')  # 受害者信息
                if victim:
                    victim_rank = victim.get('rank')  # 受害者排名
                    victim_nickname = victim.get('user').get('nickname')  # 受害者昵称
                else:
                    victim_rank = ''
                    victim_nickname = ''
                user_death_info = {
                    ...
                    'victim_nickname': victim_nickname,
                }
                user_deaths.append(user_death_info)
        else:
            user_deaths = []
        pubg_user_death = {
            'match_id': match_id,
        ...
        pubg_user_death = json.dumps(pubg_user_death, ensure_ascii=False)
        # DEATH_DATA_QUEUE.put(pubg_user_death)
        r.lpush('spider:python:pubg_death:keyword:dest', pubg_user_death)

    except BaseException as e:
        print 'pubg_death方法中出现BaseException异常', e
